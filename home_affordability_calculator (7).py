...
loan_limits = {"conforming": 832750, "high_balance": 1249129},
...
def display_loan_summary(sale_price, loan_amt, cash_close, monthly_pmt, total_pmt):
    st.write(f"Total Sale Price: ${sale_price:,.2f}")
    st.write(f"Loan Amount: ${loan_amt:,.2f}")
    st.write(f"Cash to Close: ${cash_close:,.2f}")
    st.write(f"Monthly Payment: ${monthly_pmt:,.2f}")
    st.write(f"Total Monthly Payment (Including Taxes & Insurance): ${total_pmt:,.2f}")
...
if st.session_state.button_clicked:
    ...
    # Check if it's a conforming loan and exceeds conforming limit
    if selected_formula.startswith("C") and loan_amount > loan_limits[num_units]["conforming"]:
        show_original_results = False
        st.error(f"Loan amount (${loan_amount:,.2f}) exceeds the conforming limit for {num_units}-unit property (${loan_limits[num_units]['conforming']:,.2f}).")
        ...
        if st.button(f"✅ Apply {adjusted_down_payment_pct:.2f}% Down Payment & Recalculate\nTotal Cash to Close: ${new_cash_to_close:,.2f}"):
            ...
            display_loan_summary(total_sale_price, loan_amount, cash_to_close, monthly_payment, total_monthly_payment)
    ...
    if loan_amount > max_loan_limit:
        show_original_results = False
        st.error(f"Loan amount (${loan_amount:,.2f}) exceeds the max loan limit (${max_loan_limit:,.2f}).")
    ...
    # Only display results if they're eligible
