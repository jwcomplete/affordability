...
if st.session_state.button_clicked:
    ...
    if selected_formula.startswith("C") and loan_amount > loan_limits[num_units]["conforming"]:
        ...
        if next_formula:
            ...
            if st.button(f"🔄 Switch to {next_formula} (Eligible Formula)\nTotal Cash to Close: ${new_cash_to_close_next:,.2f}"):
                ...
                total_sale_price, loan_amount, cash_to_close, monthly_payment, total_monthly_payment = calculate_loan(
                    purchase_price, loan term, interest_rate, down_payment_pct, loan_formulas[next_formula]["seller_concession"] / 100, property_tax, home_insurance, flood_insurance
                )
    ...
    # Validate high balance formulas
    elif selected_formula.startswith("HB") and (loan_amount <= loan_limits[num_units]["conforming"] or loan_amount > loan limits[num_units]["high_balance"]):
        st.markdown(f'<div style="background-color:red; color:white; padding:10px; font-size:16px;">'
        ...
        next_formula = None
        for key, values in loan_formulas.items():
            if key != selected_formula and key.startswith("HB") and (loan_limits[num_units]["conforming"] < purchase price * (1 - values["down_payment"] / 100) <= loan_limits[num_units]["high_balance"]):
                next_formula = key
                break
        ...
        if next_formula:
            ...
            if st.button(f"🔄 Switch to {next_formula} (Eligible Formula)\nTotal Cash to Close: ${new_cash_to_close_next:,.2f}"):
                selected_formula = next_formula
                down_payment_pct = loan_formulas[next_formula]["down_payment"] / 100
                total_sale_price, loan amount, cash_to_close, monthly_payment, total_monthly_payment = calculate_loan(
                    purchase_price, loan term, interest rate, down_payment_pct, loan_formulas[next_formula]["seller_concession"] / 100, property tax, home insurance, flood insurance
                )

                if total_sale_price and loan amount and cash_to_close and monthly payment and total_monthly_payment:
                    st.write(f"Total Sale Price: ${total_sale_price:,.2f}")
                    st.write(f"Loan Amount: ${loan_amount:,.2f}")
                    st.write(f"Cash to Close: ${cash_to_close:,.2f}")
                    st.write(f"Monthly Payment: ${monthly_payment:,.2f}")
    ...
    if loan amount > max_loan_limit:
        ...
        adjusted_down_payment = ((loan amount - max_loan_limit) / total_sale_price * 100) + loan_formulas[selected_formula]["down_payment"]
        ...
        if st.button(f"✅ Apply {adjusted_down_payment:.2f}% Down Payment & Recalculate\nTotal Cash to Close: ${new_cash_to_close:,.2f}"):
            ...
            total_sale_price, loan amount, cash_to_close, monthly payment, total_monthly_payment = calculate_loan(
                purchase price, loan term, interest rate, down_payment_pct, seller concession_pct, property tax, home insurance, flood insurance
            )

            if total_sale_price and loan amount and cash to_close and monthly payment and total_monthly_payment:
                st.write(f"Total Sale Price: ${total_sale_price:,.2f}")
                st.write(f"Loan Amount: ${loan_amount:,.2f}")
                st.write(f"Cash to Close: ${cash_to_close:,.2f}")
                st.write(f"Monthly Payment: ${monthly_payment:,.2f}")
        ...
        next_formula = None
        for key, values in loan formulas.items():
            if key != selected_formula and (purchase price * (1 - values["down_payment"] / 100)) <= max_loan_limit:
                next_formula = key
                break

        if next_formula:
            new_cash_to_close_next = total_sale_price * (loan formulas[next_formula]["down_payment"] / 100)
            if st.button(f"🔄 Switch to {next_formula} (Eligible Formula)\nTotal Cash to Close: ${new_cash_to_close_next:,.2f}"):
                selected_formula = next_formula
                down_payment_pct = loan_formulas[next_formula]["down_payment"] / 100
                total_sale_price, loan amount, cash_to_close, monthly payment, total_monthly_payment = calculate_loan(
                    purchase price, loan term, interest rate, down_payment_pct, loan formulas[next_formula]["seller_concession"] / 100, property tax, home insurance, flood insurance
                )

                if total_sale_price and loan amount and cash to_close and monthly payment and total_monthly_payment:
                    st.write(f"Total Sale Price: ${total_sale_price:,.2f}")
                    st.write(f"Loan Amount: ${loan_amount:,.2f}")
                    st.write(f"Cash to Close: ${cash to_close:,.2f}")
                    st.write(f"Monthly Payment: ${monthly payment:,.2f}")
                    st.write(f"Total Monthly Payment (Including Taxes & Insurance): ${total_monthly_payment:,.2f}")
